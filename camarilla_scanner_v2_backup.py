...
# Filter 2 — Proximity: price must be very close to L3/H3
MAX_DIST_PCT       = 0.3   # Within 0.3% of L3 (bull) or H3 (bear)
                            # Only genuinely actionable setups
...
MIN_VOL_RATIO      = 1.5   # Today's volume >= 1.5x 20-day average
                            # Confirms genuine interest at the level

# Filter 6 — SL risk cap
MAX_SL_PCT         = 4.0   # Stop loss must be within 4% of entry
                            # Wider stops = lower probability setups

# Filter 7 — No sideways stocks
# Stocks appearing in BOTH bull and bear watchlist are skipped
# Price stuck between L3 and H3 = no clear directional bias

# Filter 8 — Virgin level (most powerful filter from book)
# L3/H3 must NOT have been touched earlier this week
# First touch = highest probability, subsequent = lower
...
def apply_filters(conn, week_start, levels):
    ...
    log(f"  After all filters    → Bull:{len(bullish)} Bear:{len(bearish)}")
