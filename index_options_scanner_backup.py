...
def pick_best_setup(levels):
    ...
    for sym, lv in levels.items():
        ...
        ema    = lv.get('ema20w') or 0
        is_bull = price >= ema * 0.97

        # CALL setup — only if uptrend or marginally below EMA
        dist_l3  = abs(price - lv['l3']) / price * 100
        dist_h3  = abs(price - lv['h3']) / price * 100

        if is_bull:
            # Prefer CALL in uptrend
            candidates.append({
                'sym':       sym,
                'direction': 'CALL',
                'dist_pct':  dist_l3,
        ...
        # Also add the counter-trend setup if VERY close (within 0.5%)
        if is_bull and dist_h3 <= 0.5:
            candidates.append({
                'sym':       sym,
                'direction': 'PUT',
                'dist_pct':  dist_h3,
        ...
        elif not is_bull and dist_l3 <= 0.5:
            candidates.append({
                'sym':       sym,
                'direction': 'CALL',
                'dist_pct':  dist_l3,
...
def monitor():
    ...
    for sym, lv in levels.items():
        ...
        if (high >= h3 * 0.998 and close < h3 and close < op):
            log(f"  PUT S1: H3 rejection")
            send_telegram(
...
"
                f"H3 Rejection
"
                f"Touched H3 ({h3:,.0f}) closed below
...
"
        ...
        # Signal 2: H4 rejection
        elif (high >= h4 * 0.998 and close < h4 and close < op):
            log(f"  PUT S2: H4 rejection")
            send_telegram(
...
"
                f"H4 Rejection (Strong!)
"
                f"Touched H4 ({h4:,.0f}) closed below
...
"
                f"BUY PUT NOW — Strong Setup!"
            )

        # Signal 3: H4 breakout WARNING
        elif (close > h4 and prev_close <= h4):
            log(f"  WARNING: broke above H4")
            send_telegram(
                f"⚠️ BREAKOUT WARNING: {info['name']}
"
                f"{ist_time}

"
                f"Broke ABOVE H4 ({h4:,.0f})
"
                f"Current: {close:,.0f}
...
"
                f"NO TRADE YET
"
                f"Watch for reversal below H4
"
                f"False breakout = Strong PUT"
            )
...
"
        ...
        # Signal 5: L3 bounce
        if (low <= l3 * 1.002 and close > l3 and close > op):
            log(f"  CALL S5: L3 bounce")
            send_telegram(
...
"
                f"L3 Bounce
"
                f"Touched L3 ({l3:,.0f}) closed above
...
"
        ...
        # Signal 6: L4 bounce
        elif (low <= l4 * 1.002 and close > l4 and close > op):
            log(f"  CALL S6: L4 bounce")
            send_telegram(
...
"
                f"L4 Bounce (Strong!)
"
                f"Touched L4 ({l4:,.0f}) closed above
...
"
        ...
        # Signal 7: L4 breakdown WARNING
            ...
            send_telegram(
                f"⚠️ BREAKDOWN WARNING: {info['name']}
"
                f"{ist_time}
