...
# ── Trading config ─────────────────────────────────────────────
CAPITAL          = 60000   # Rs 60,000 total capital (3 positions)
...
DAILY_LOSS_LIMIT = 3000   # 3 positions x Rs 1,000 max loss
MONTHLY_LOSS_LIMIT = 10000 # ~1 bad month buffer (backtest worst: +Rs 2,151)
...
def place_trade(symbol, entry, sl, target, signal_source='HV', w_h3=None, w_h4=None, target2=None):
    ...
    try:
        ...
        # Always LIMIT order at L3 — matches backtest exactly
        order_type  = 'LIMIT'
        order_price = str(round(entry, 2))
        log(f"  Price {current_price:.2f} | L3 {entry} | {pct_from_l3:+.1f}% — LIMIT @ Rs {entry}")
        if pct_from_l3 > 2.0:
            log(f"  Price {pct_from_l3:.1f}% above L3 — too far, skipping")
            send_telegram(
                f"SIGNAL SKIPPED: {symbol}\n"
                f"Price Rs {current_price:.2f} is {pct_from_l3:.1f}% above L3\n"
                f"L3 entry: Rs {entry:.2f}\n"
                f"Too far to chase — wait for pullback"
            )
            return {'status': 'SKIPPED', 'reason': 'price_too_far_from_l3'}
        ...
        msg = (
            ...
            f"Type   : {order_type}\n"
            f"\n"
            f"Shares : {shares} @ Rs {position_value:,.0f}\n"
            f"Risk   : Rs {risk_amount:,.0f}\n"
            f"Reward : Rs {reward_amount:,.0f}\n"
            f"\n"
            f"Waiting for fill → GTT SL+Target auto after fill"
        )
        send_telegram(msg)
...
# ═══════════════════════════════════════════════════════════════
#  PLACE GTT TRADE — EOD VERSION (4:15 PM)
#  Places ALL 3 GTTs at once — no market hours restriction!
#  GTT BUY at L3 + GTT SL at L4 + GTT Target at HV High
# ═══════════════════════════════════════════════════════════════
def place_gtt_trade(symbol, entry, sl, target, signal_source='HV_EOD'):
    """
    Place complete GTT trade at EOD (4:15 PM):
      1. GTT BUY  at L3 (valid 365 days)
      2. GTT SL   at L4 (valid 365 days)
      3. GTT Target at HV High (valid 365 days)
    No market hours restriction — GTT works anytime!
    """
    import time
    ist_time = get_ist().strftime('%d %b %Y %H:%M IST')
    log(f"GTT Trade: {symbol} | Entry:{entry} | SL:{sl} | Target:{target}")
    ...
    daily_pnl = sum(t.get('exit_pnl', 0) for t in trades_check)
    if daily_pnl <= -DAILY_LOSS_LIMIT:
        log(f"Daily loss limit hit — no new trades")
        return {'status': 'BLOCKED', 'reason': 'daily_loss_limit'}

    # Check monthly loss limit
    try:
        if not check_monthly_loss():
            log(f"Monthly loss limit hit — no new trades")
            return {'status': 'BLOCKED', 'reason': 'monthly_loss_limit'}
    except Exception as e:
    ...
    open_positions = get_open_positions()
    if len(open_positions) >= MAX_OPEN_POS:
        log(f"Max positions ({MAX_OPEN_POS}) reached — skip {symbol}")
        return {'status': 'SKIPPED', 'reason': 'max_positions'}

    # Check already traded or has open position
    if any(t['symbol'] == symbol for t in trades_check):
        log(f"{symbol}: already in trades — skip")
        return {'status': 'SKIPPED', 'reason': 'already_traded'}

    # Check existing GTTs on Angel One (avoid duplicates!)
    try:
        sa, _ = get_smart_api()
        existing_gtts = sa.gttLists(
            status=["NEW","ACTIVE","SENTTOEXCHANGE"],
            page=1, count=50
        )
        if existing_gtts.get('data'):
            token_check, ts_check = get_symbol_token(symbol)
            for gtt in existing_gtts['data']:
                if (gtt.get('tradingsymbol','').replace('-EQ','') == symbol or
                    gtt.get('symboltoken') == token_check):
                    log(f"{symbol}: already has GTT #{gtt['id']} — skip duplicate")
                    return {'status': 'SKIPPED', 'reason': 'gtt_already_exists'}
    except Exception as e:
        log(f"GTT duplicate check failed: {e} — proceeding")

    # ── Position sizing ────────────────────────────────────────
    sl_distance = round(entry - sl, 2)
    if sl_distance <= 0:
        log(f"Invalid SL distance: {sl_distance}")
        return {'status': 'ERROR', 'reason': 'invalid_sl'}

    shares = min(
        int(RISK_PER_TRADE / sl_distance),
        int(MAX_POSITION_VAL / entry)
    )
    if shares < 1:
        log(f"{symbol}: position too small")
        return {'status': 'ERROR', 'reason': 'position_too_small'}
    ...
    log(f"  Shares:{shares} | Value:Rs{position_value} | Risk:Rs{risk_amount} | Reward:Rs{reward_amount}")
    ...
    if not token:
        log(f"Token not found for {symbol}")
        send_telegram(
            f"⚠️ GTT ERROR: {symbol}\n"
            f"Token not found — place manually:\n"
            f"BUY {shares} @ Rs {entry} | SL Rs {sl} | T Rs {target}"
        )
        return {'status': 'ERROR', 'reason': 'token_not_found'}
    ...
    try:
        ...
        # GTT 1: BUY at L3
        buy_price = round(entry * 1.002, 2)  # 0.2% above trigger for fill
        gtt_buy_params = {
            "tradingsymbol" : trading_symbol,
            "symboltoken"   : token,
            "exchange"      : "NSE",
        ...
        log(f"  Placing GTT BUY: {trading_symbol} x{shares} @ trigger Rs {entry}")
        buy_response = smart_api.gttCreateRule(gtt_buy_params)
        log(f"  GTT BUY response: {buy_response}")

        # Extract GTT BUY ID
        if isinstance(buy_response, (str, int)) and str(buy_response).isdigit():
            gtt_buy_id = str(buy_response)
        elif isinstance(buy_response, dict):
            gtt_buy_id = str(buy_response.get("data", {}).get("id", ""))
        else:
            gtt_buy_id = ""

        if not gtt_buy_id:
            log(f"  GTT BUY failed: {buy_response}")
            send_telegram(
                f"⚠️ GTT BUY FAILED: {symbol}\n"
                f"Place manually: BUY {shares} @ Rs {entry}\n"
                f"SL: Rs {sl} | Target: Rs {target}"
            )
            return {'status': 'ERROR', 'reason': 'gtt_buy_failed'}

        log(f"  ✅ GTT BUY placed: ID {gtt_buy_id}")
        time.sleep(1)
        ...
        gtt_sl_params = {
            ...
            "producttype"   : "DELIVERY",
            "transactiontype": "SELL",
            "price"         : str(round(sl * 0.998, 2)),
            "qty"           : str(shares),
            "triggerprice"  : str(sl),
            "disclosedqty"  : str(shares),
            "timeperiod"    : 365,
        }
        log(f"  Placing GTT SL: {trading_symbol} x{shares} @ trigger Rs {sl}")
        sl_response = smart_api.gttCreateRule(gtt_sl_params)
        log(f"  GTT SL response: {sl_response}")

        if isinstance(sl_response, (str, int)) and str(sl_response).isdigit():
            gtt_sl_id = str(sl_response)
        elif isinstance(sl_response, dict):
            gtt_sl_id = str(sl_response.get("data", {}).get("id", ""))
        else:
            gtt_sl_id = ""

        log(f"  ✅ GTT SL placed: ID {gtt_sl_id}")
        time.sleep(1)

        # GTT 3: Target at HV High
        gtt_tgt_params = {
            "tradingsymbol" : trading_symbol,
            "symboltoken"   : token,
            "exchange"      : "NSE",
            "producttype"   : "DELIVERY",
            "transactiontype": "SELL",
            "price"         : str(round(target * 0.998, 2)),
            "qty"           : str(shares),
            "triggerprice"  : str(target),
            "disclosedqty"  : str(shares),
            "timeperiod"    : 365,
        }
        log(f"  Placing GTT Target: {trading_symbol} x{shares} @ trigger Rs {target}")
        tgt_response = smart_api.gttCreateRule(gtt_tgt_params)
        log(f"  GTT Target response: {tgt_response}")

        if isinstance(tgt_response, (str, int)) and str(tgt_response).isdigit():
            gtt_tgt_id = str(tgt_response)
        elif isinstance(tgt_response, dict):
            gtt_tgt_id = str(tgt_response.get("data", {}).get("id", ""))
        else:
            gtt_tgt_id = ""

        log(f"  ✅ GTT Target placed: ID {gtt_tgt_id}")

        # ── Save trade record ──────────────────────────────────
        trade_record = {
            'symbol'        : symbol,
            'order_id'      : gtt_buy_id,
            'gtt_buy_id'    : gtt_buy_id,
            'gtt_sl_id'     : gtt_sl_id,
            'gtt_target_id' : gtt_tgt_id,
            'order_type'    : 'GTT',
            'entry'         : entry,
            'sl'            : sl,
            'target'        : target,
            ...
            'signal_source' : signal_source,
            'placed_at'     : ist_time,
            'sl_order_id'   : gtt_sl_id,
            'leg1_done'     : False,
            'leg1_shares'   : shares // 2,
            'leg2_shares'   : shares - shares // 2,
        }
        trades_today.append(trade_record) if False else None
        all_trades = load_trades()
        all_trades.append(trade_record)
        save_trades(all_trades)

        # ── Send Telegram ──────────────────────────────────────
        next_day = (get_ist() + __import__('datetime').timedelta(days=1)).strftime('%d %b %Y')
        # Skip weekends
        from datetime import datetime, timedelta
        nd = get_ist().date() + timedelta(days=1)
        while nd.weekday() >= 5:
            nd += timedelta(days=1)
        next_trading = nd.strftime('%d %b %Y')

        msg = (
            f"✅ GTT TRADE PLACED: {symbol}\n"
            f"{ist_time}\n"
            f"\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"GTT BUY    : Rs {entry} (L3) #{gtt_buy_id}\n"
            f"GTT SL     : Rs {sl} (L4) #{gtt_sl_id}\n"
            f"GTT Target : Rs {target} (HV High) #{gtt_tgt_id}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"Shares : {shares}\n"
            ...
            f"R:R    : 1:{rr}\n"
            f"\n"
            f"Active from: {next_trading}\n"
            f"Valid for  : 365 days\n"
            f"\n"
            f"All 3 GTTs auto-execute. No action needed!"
        )
        send_telegram(msg)

        log(f"  ✅ Complete GTT trade placed for {symbol}")
        return {
            'status'        : 'GTT_PLACED',
            'gtt_buy_id'    : gtt_buy_id,
            'gtt_sl_id'     : gtt_sl_id,
        ...
        log(f"GTT trade error: {e}")
        send_telegram(
            f"❌ GTT ERROR: {symbol}\n"
            f"Error: {e}\n"
            f"Place manually:\n"
            f"BUY {shares} @ Rs {entry}\n"
            f"SL: Rs {sl} | Target: Rs {target}"
        )
        return {'status': 'ERROR', 'reason': str(e)}
...
def check_positions():
    ...
    try:
        ...
        for t in trades:
            ...
            # ── PENDING: Check if MARKET/LIMIT order was filled ──
            if status in ('OPEN', 'GTT_PENDING'):
                if sym in held_symbols and held_symbols[sym]['qty'] > 0:
                    actual_entry = held_symbols[sym]['avg']
                    # First time fill detection — place GTTs
                    if not t.get('sl_order_id'):
                        log(f"  {sym}: ORDER FILLED @ Rs {actual_entry:.2f}!")
                        t['status']    = 'OPEN'
                        t['entry']     = actual_entry
                        t['filled_at'] = ist_time
                        updated = True
                        # Place SL GTT + Target GTT
                        ...
                        # Already filled and GTTs placed — just monitor
                        ltp     = held_symbols[sym]['ltp']
                        remaining = held_symbols[sym]['qty']
                        pnl     = round((ltp - actual_entry) * remaining, 0)
                        pnl_pct = round((ltp - actual_entry) / actual_entry * 100, 1)
                        log(f"  {sym}: HOLDING | LTP={ltp:.2f} | Entry={actual_entry:.2f} | P&L Rs {pnl:+,} ({pnl_pct:+.1f}%) | Target={target:.2f} | SL={sl:.2f}")
                else:
                    # Not in holdings — order pending or SL hit
                    if t.get('sl_order_id'):
                        # GTTs were placed — shares gone means SL or Target hit
                        log(f"  {sym}: shares gone — GTT triggered (SL or Target)")
                        t['status']   = 'CLOSED'
                        t['exit_date']= str(date.today())
                        updated = True
                        # Determine if SL or Target by comparing ltp
                        try:
                            _ltp = float(smart_api.ltpData('NSE', sym+'-EQ', token).get('data',{}).get('ltp', 0))
                        except:
                            _ltp = 0
                        _is_target = _ltp >= t.get('target', 0) * 0.98 if t.get('target') else False
                        _pnl = round((_ltp - t.get('entry', _ltp)) * shares, 0) if _ltp > 0 else 0
                        # ── Cancel the other GTT ──────────────────────
                        if _is_target:
                            # Target hit → cancel SL GTT
                            _sl_gtt = t.get('sl_order_id')
                            if _sl_gtt:
                                try:
                                    cancel_gtt(_sl_gtt, sym+'-EQ', token)
                                    log(f"  Cancelled SL GTT {_sl_gtt} (target hit)")
                                except Exception as _e:
                                    log(f"  Could not cancel SL GTT: {_e}")
                            send_telegram(
                                f"TARGET HIT: {sym}\n"
                                f"{ist_time}\n"
                                f"Entry  : Rs {entry:.2f}\n"
                                f"Target : Rs {t.get('target',0):.2f}\n"
                                f"P&L    : Rs {_pnl:+,.0f}\n"
                                f"SL GTT cancelled automatically\n"
                                f"Position fully closed"
                            )
                        else:
                            # SL hit → cancel Target GTT
                            _tgt_gtt = t.get('target_gtt_id')
                            if _tgt_gtt:
                                try:
                                    cancel_gtt(_tgt_gtt, sym+'-EQ', token)
                                    log(f"  Cancelled Target GTT {_tgt_gtt} (SL hit)")
                                except Exception as _e:
                                    log(f"  Could not cancel Target GTT: {_e}")
                            send_telegram(
                                f"SL HIT: {sym}\n"
                                f"{ist_time}\n"
                                f"Entry : Rs {entry:.2f}\n"
                                f"SL    : Rs {sl:.2f}\n"
                                f"P&L   : Rs {_pnl:+,.0f}\n"
                                f"Target GTT cancelled automatically\n"
                                f"Position closed"
                            )
                    else:
                        # Order not filled yet
                        try:
                            ltp = float(smart_api.ltpData('NSE', sym+'-EQ', token).get('data',{}).get('ltp', entry))
                        except:
                            ltp = entry
                        pct_away = (ltp - entry) / entry * 100 if entry > 0 else 0
                        log(f"  {sym}: Order pending | LTP={ltp:.2f} | L3={entry:.2f} | {pct_away:+.1f}%")
                        if pct_away > 3.0 and ist.hour >= 12:
                            send_telegram(
                                f"ORDER NOT FILLED: {sym}\n"
                                f"Limit @ Rs {entry:.2f}\n"
                                f"Current Rs {ltp:.2f} ({pct_away:+.1f}% away)\n"
                                f"Consider cancelling if no longer valid."
                            )
                continue

            if status not in ('OPEN', 'GTT_PENDING'):
                continue

        # ── Check for manual trades ───────────────────────────
        for sym, h in held_symbols.items():
            # Skip if already tracked in ANY status
            already = any(t.get('symbol') == sym for t in trades)
            if not already and h['qty'] > 0:
                log(f"  Untracked holding: {sym} {h['qty']} shares @ Rs {h['avg']:.2f} — alerting only, no auto GTT")
                send_telegram(
                    f"UNTRACKED HOLDING: {sym}\n"
                    f"{h['qty']} shares @ Rs {h['avg']:.2f}\n"
                    f"Not in scanner records.\n"
                    f"Please check Angel One and set GTT manually."
                )
