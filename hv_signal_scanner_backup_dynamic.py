...
def scan(test_mode=False):
    ...
    for row in candidates:
        ...
        if vol_ratio < MIN_VOL_RATIO:
            ...
            continue
        # Filter: HV day strength cap (avoid news/event spikes)
        hv_vol_ratio = hv_vol / avg_vol if avg_vol > 0 else 0
        if hv_vol_ratio > MAX_HV_VOL_RATIO:
            log(f'  {sym}: HV too strong ({hv_vol_ratio:.1f}x > {MAX_HV_VOL_RATIO}x) — skip')
            continue
        ...
        if rr < MIN_RR:
            ...
            continue
        if rr > MAX_RR:
            log(f'  {sym}: R:R too high ({rr:.1f} > {MAX_RR}) -- skip')
            continue
    ...
    # Check monthly loss limit before placing any trades
    if auto_trade:
        try:
            from angel_trader import check_monthly_loss
            if not check_monthly_loss():
                log("Monthly loss limit hit — no trades this month")
                send_telegram(
                    f"HV SCANNER: {len(signals)} signal(s) found\n"
                    f"Monthly loss limit reached — no trades placed.\n"
                    f"Signals: {', '.join(s['symbol'] for s in signals)}"
                )
                return
        except Exception as e:
            log(f"Monthly loss check failed: {e} — proceeding")
    for sig in signals:
        sym = sig['symbol']
    ...
    if len(signals) > 1:
        ...
        for sig in signals:
            summary_lines.append(
                ...
                f"HVAge:{sig.get('hv_age','?')}d"
            )
        summary_lines.append("")
        summary_lines.append(f"Filters: HV<=60d | Vol>=1.5x | RR 5-20x")
        summary_lines.append(f"Auto-trading top signal. Others skipped if slots full.")
