...
RISK_PER_TRADE   = 500    # Phase 1    # Rs 750 risk per trade (3% of Rs 25k)
...
def place_trade(symbol, entry, sl, target, signal_source='HV', w_h3=None, w_h4=None, target2=None):
    ...
    try:
        ...
        leg1_rr = round((w_h3 - entry) / (entry - sl), 1) if w_h3 and (entry - sl) > 0 else 0
        leg2_rr = round((t2 - entry) / (entry - sl), 1) if (entry - sl) > 0 else 0

        msg = (
            f"✅ ORDER PLACED: {symbol}\n"
            f"{ist_time}\n"
            f"\n"
            f"HV + Daily Camarilla Signal\n"
            ...
            f"\n"
            f"Order ID: {order_id}\n"
            f"GTT SL will be placed after fill\n"
            f"Auto-exit at HV High ✅"
        )
        send_telegram(msg)
        ...
        for t in trades:
            ...
            if status == 'GTT_PENDING':
                if sym in held_symbols and held_symbols[sym]['qty'] > 0:
                    ...
                    # Place OCO GTT (SL + Target in one rule)
                    gtt_oco_id = place_gtt_oco(smart_api, sym+'-EQ', token,
                                               shares, actual_entry, sl, target)
                    if gtt_oco_id:
                        t['sl_order_id']   = gtt_oco_id
                        t['target_gtt_id'] = gtt_oco_id
                        log(f"  GTT OCO live - SL Rs {sl:.2f} | Target Rs {target:.2f}")
                    else:
                        log(f"  GTT OCO FAILED - set manually on Angel One!")
                    send_telegram(
                        f"ORDER FILLED: {sym}\n"
                        f"{ist_time}\n"
